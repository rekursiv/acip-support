...
public class PageSizeEntryRow extends EntryCell {


	public PageSizeEntryRow(Composite parent) {
		super(parent, "Page Size");
		addHorizSep();
		...
		TextEntryCell width = new TextEntryCell(this, "Width", 50, BoxType.SIMPLE, 100) {
			...
			protected void getModelData() {
				if (ctlr.getModel().pageSize==null) data = null;   // FIXME:  this should be done in the model
				else data = String.valueOf(ctlr.getModel().pageSize.width);
			}
			
			@Override
			protected void setModelData() {
				if (ctlr.getModel().pageSize==null) ctlr.getModel().pageSize = new SizeModel();  // FIXME:  this should be done in the model
				ctlr.getModel().pageSize.width = Integer.parseInt(data);
			}
		};
		...
		TextEntryCell height = new TextEntryCell(this, "Height", 50, BoxType.SIMPLE, 100) {
			...
			protected void getModelData() {
				if (ctlr.getModel().pageSize==null) data = null;   // FIXME:  this should be done in the model
				else data = String.valueOf(ctlr.getModel().pageSize.height);
			}
			
			@Override
			protected void setModelData() {
				if (ctlr.getModel().pageSize==null) ctlr.getModel().pageSize = new SizeModel();  // FIXME:  this should be done in the model
				ctlr.getModel().pageSize.height = Integer.parseInt(data);
			}
		};
